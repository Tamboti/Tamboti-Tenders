// Single source of truth for the app logotype mark — used by PublicNav and
// the admin/portal Sidebar so a future logo swap only happens in one place.
export const LOGO_URL =
  "https://gdbodrzxdbtskyzmqmuu.supabase.co/storage/v1/object/public/Company%20assets/d7d592e6-4c11-47a1-888d-f2c924958a69-removebg-preview.png";

// The product was renamed from "Tender Compass" to "Tamboti Tenders" — this
// is the single source of truth so SEO metadata, alt text, toasts, and
// emails all stay in sync with whatever the nav/footer display.
export const SITE_NAME = "Tamboti Tenders";
